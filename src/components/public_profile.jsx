...
export default function PublicProfile() {
  ...
  const fetchPublicProfile = useCallback(async () => {
    ...
    // 1. Fetch Data User & Setting Privacy
    ...
    if (userError) {
      console.error("Gagal mengambil data profile: ", userError.message);
      setLoading(false);
      return;
    }
    ...
    // 2. Fetch Posts (Selalu ambil data, samarkan jika hide_history / is_anonim_mode bernilai true)
    ...
    if (!postError && publicPosts) {
      const formattedPosts = publicPosts.map((p) => {
        const isHide = user.hide_history || user.is_anonim_mode || p.is_anonim_mode;
        return {
          ...p,
          users: isHide
            ? { username: "Anonim", avatar_url: null }
            : p.users || { username: user.username, avatar_url: user.avatar_url },
        };
      });
      setPosts(formattedPosts);
    }

    // 3. Fetch Comments (Samarkan penulisnya jika hide_comment / is_anonim_mode bernilai true)
    ...
    if (!commentError && commentsData) {
      const formattedComments = commentsData.map((c) => {
        const isHide = user.hide_comment || user.is_anonim_mode;
        return {
          ...c,
          user_is_anonim: isHide,
        };
      });
      setUserComment(formattedComments);
    }

    // 4. Fetch Votes (Samarkan jika hide_vote / is_anonim_mode bernilai true)
    ...
    if (!voteError && voteData) {
      const formattedVotes = voteData.map((v) => {
        const isHide = user.hide_vote || user.is_anonim_mode;
        return {
          ...v,
          user_is_anonim: isHide,
        };
      });
      setUserVote(formattedVotes);
  ...
  const handleVoteClick = async (voteItem) => {
    if (!voteItem) return;
    if (voteItem.posts) {
      setSelectedpost(voteItem.posts);
      setSelectedcomment(null);
  ...
  if (!userData) return <div className="p-10 text-center text-gray-500">Pengguna tidak ditemukan.</div>;
      ...
      <div className="max-w-4xl mx-auto flex flex-col gap-6">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => window.history.back()} className="p-2 rounded-full cursor-pointer">
            <svg className="w-6 h-6 stroke-[#a50034] dark:stroke-[#f1ece1]" fill="none" strokeWidth="2.5" viewBox="0 0 24 24">
          ...
          <div className="px-8 pb-8 relative">
            <div className="-mt-14 mb-4 relative inline-block">
              <div className="w-28 h-28 border-4 border-white dark:border-[#1e1e1e] rounded-full shadow-md bg-white dark:bg-[#1e1e1e] overflow-hidden flex items-center justify-center">
                {userData.avatar_url ? (
                  <img src={userData.avatar_url} alt="Foto profil" className="w-full h-full rounded-full object-cover" />
                ) : (
            ...
            <h2 className="text-2xl font-bold text-gray-900 dark:text-[#f1ece1]">{userData.username || "User"}</h2>
            ...
            <div className="flex flex-col gap-1 mt-2">
              ...
              <p className="text-gray-700 dark:text-[#f1ece1] text-sm leading-relaxed">{userData.bio || "Belum ada deskripsi"}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-[#1e1e1e] rounded-2xl min-h-[200px] flex items-center justify-center p-8">
                ...
                {posts.map((item) => (
                  ...
                  >
                    <div className="h-48 w-full overflow-hidden rounded-lg flex justify-center items-center bg-[#f1ece1]">
                      {item.image_url ? (
                        <img src={item.image_url} alt="Post media" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                      ) : (
                        <p className="text-[#a50034] font-semibold text-center text-xl line-clamp-4">#{item.tag}</p>
                      )}
                    </div>
                  </div>
                ))}
          ...
          {activeTab === "comments" &&
            (userComment.length === 0 ? (
              <p className="text-gray-400 dark:text-[#f1ece1] text-sm font-medium">Belum ada komentar yang dibuat.</p>
            ) : (
              <div className="flex flex-col gap-3 w-full">
                {userComment.map((item) => (
                  ...
                  >
                    ...
                    {item.user_is_anonim && (
                      <span className="text-[10px] w-fit font-bold bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-2 py-0.5 rounded mt-1">
          ...
          {activeTab === "polling" &&
            (userVote.length === 0 ? (
              <p className="text-gray-400 dark:text-[#f1ece1] text-sm font-medium">Belum ada kontribusi vote.</p>
            ) : (
              <div className="flex flex-col gap-3 w-full">
                {userVote.map((item) => (
                  <div
