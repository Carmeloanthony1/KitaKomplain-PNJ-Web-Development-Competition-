    ...
    async function fetchUserData() {
      ...
      try {
        ...
        const { data: postsData } = await supabase
          .from("posts")
          .select("id, description, image_url, tag, created_at")
          .eq("user_id", userId)
          .order("created_at", { ascending: false });
        setUserPosts(postsData || []);

        // 3. Fetch Comments (JOIN ke tabel posts untuk ambil kolom 'tag')
        ...
        // 4. Fetch Votes (JOIN ke tabel posts untuk ambil kolom 'tag')
                    ...
                    {userComments.length === 0 ? (
                      ...
                      <div className="w-full flex flex-col gap-3">
                        {userComments.map((comment) => (
                          <div 
                            key={comment.id}
                            className="w-full bg-white border border-gray-200 rounded-xl p-3 shadow-xs text-left"
                          >
                            <div className="flex justify-between items-center text-[11px] text-gray-400 mb-1">
                              {/* Menampilkan Tag Postingan */}
                    ...
                    {userVotes.length === 0 ? (
                      ...
                      <div className="w-full flex flex-col gap-3">
                        {userVotes.map((vote) => (
                          <div 
                            key={vote.id}
                            className="w-full bg-white border border-gray-200 rounded-xl p-3 shadow-xs text-left flex justify-between items-center"
                          >
                            <div className="flex items-center gap-2">
                              ...
                              {/* Menampilkan Tag Postingan menggantikan Post ID */}
